...
void ft_isprint_test(void)
{
    ...
    {
        total_tests++;
        int expected = (c >= 32 && c <= 126) ? 1 : 0;  // Printable ASCII range is 32 to 126
    ...
    {
        total_tests++;
        int expected = 0;  // ft_isprint should return 0 for extended ASCII
        int result = ft_isprint(c);
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_isprint(%d) = %d, expected %d\n", 
    ...
    {
        total_tests++;
        int expected = 0;  // ft_isprint should return 0 for negative values
        int result = ft_isprint(c);
        if (result != expected)
        {
            printf("%s[FAIL]%s ft_isprint(%d) = %d, expected %d\n", 
